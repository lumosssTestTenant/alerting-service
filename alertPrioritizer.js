 ...
 *
 * Scores and prioritizes alerts based on business impact. Subscription
 * grace period failures get boosted priority because they directly
 * affect customer access and revenue.
 *
 * Relates to: RD-19
 */
...
function scoreAlert(alert) {
  ...
  const catScore = PRIORITY_WEIGHTS.category[alert.category] || PRIORITY_WEIGHTS.category.general;
  const tierScore = PRIORITY_WEIGHTS.customerTier[alert.customerTier] || 0;

  // Boost for grace period expiry — direct revenue impact
...
function sortAlerts(alerts) {
  return alerts
    .map(a => ({ ...a, _priority: scoreAlert(a) }))
